...
from bci_disc_models.utils import DEVICE
...
def simulate_typing(
    ...
    all_true_letter = np.random.randint(27, size=1000)
    for i in trange(n_chars_to_spell, desc="Letters", leave=True, position=0):
        ...
        true_letter_idx = all_true_letter[i]
        while not finished_sequence_attempts and attempts_remaining > 0:
            ...
            if not without_threshold:
                ...
                predicted_letter_idx = np.argmax(np.exp(log_probs))
...
def simulate_typing_RNN(
    ...
    for i in trange(n_chars_to_spell, desc="Letters", leave=True, position=0):
        ...
        true_letter_idx = all_true_letter[i]
        if model_size == "large":
            h_t = torch.zeros(32 * 28, dtype=torch.float, device=DEVICE)
        elif model_size == "medium":
            h_t = torch.zeros(24 * 28, dtype=torch.float, device=DEVICE)
        elif model_size == "small":
            h_t = torch.zeros(16 * 28, dtype=torch.float, device=DEVICE)
        while not finished_sequence_attempts and attempts_remaining > 0:
            attempts_remaining -= 1
            ...
            h_t, b_t, log_probs = model.eval_RNN(data, h_t, torch.tensor(queried_letter_indices))
            log_probs = log_probs.cpu().numpy().astype(np.float64)
            this_letter_log_likelihoods.append(log_probs.copy())
